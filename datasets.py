...
class AQADataset(Dataset):
    def __init__(self, dataset_name, feat_dir,
        ...
        if segments < 0:
            self.segments = 70 if dataset_name in ['Ball', 'Clubs', 'Hoop', 'Ribbon'] else 130
        else:
    ...
    def __getitem__(self, idx):
        ...
        if len(rgb_data) > self.segments:
            rgb_data = rgb_data[:self.segments]
            audio_data = audio_data[:self.segments]
            flow_data = flow_data[:self.segments]
        ...
        label_grade = min(max(int(label * 4), 0), 3)
        return rgb_data, audio_data, flow_data, label, label_grade
