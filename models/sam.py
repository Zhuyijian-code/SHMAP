...
def gsp(in_channels, num_classes, normal = False):
...
class SAM(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_head, n_encoder, n_decoder, n_query, dropout, activate_regular_restrictions):
        ...
        self.val = False
        self.score_len = 100
        etf_vec1, etf_rect = gsp(self.score_len, self.score_len)
    ...
    def forward(self, x, return_feat = False):
        ...
        s = self.regressor(q1)
        logits = torch.diagonal(s, dim1=-2, dim2=-1)
        result['int'] = logits
        ...
        if return_feat:
            return {'output': result, 'embed': q1, "other":{
                "int":logits.clone().cpu().detach().numpy() ,
    ...
    def get_score(self, x):
        cls_score1 = x['dec'][0] @ self.etf_vec1
